"""NIH NIMH autism information pages adapter.

Seeded from ``data/seed_urls.yaml`` under the ``nih-nimh:`` section.
NIMH content is a US-Government work in the public domain.
"""
...
@register_adapter
class NihNimhAdapter(SourceAdapter):
    source_id = "nih-nimh"
    licence = "public-domain"
    licence_url = "https://www.nimh.nih.gov/site-info/policies"
    attribution_template = "National Institute of Mental Health, NIH (public domain)"
    ...
    def fetch(self, ref: SourceItemRef) -> RawDocument:
        if ref.url is None:
            raise ValueError(f"NihNimhAdapter requires ref.url for {ref.item_id!r}")
